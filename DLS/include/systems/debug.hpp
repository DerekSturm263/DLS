...
namespace dls::debug::systems {
	class debug : public core::types::system<functions::out> {
        ...
        public:
            debug() : _file_path(), _output() { }

            debug(std::string const& file_path) : _file_path(file_path), _output() { }
            debug(std::string&& file_path) : _file_path(std::move(file_path)), _output() { }
...
REGISTER_SYSTEM("DebugSystem", dls::debug::systems::debug);
